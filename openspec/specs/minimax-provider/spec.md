...
MiniMax 供应商（`minimax`）的接入——一个与现有 Claude / OpenAI / Ollama / LangCat 并列的可选 AI 服务供应商，经 OpenAI 兼容 Chat Completions 协议访问 MiniMax 大模型。覆盖供应商注册与配置读取、思考开关（`thinking.type`，按 M3 / M2.x 模型族区分取值）与推理输出路由（`reasoning_split`）的正确注入、以及响应解析与用量统计。
