# claude-tool-calling Specification

## Purpose

ClaudeService SHALL use the Anthropic SDK's native tool-calling protocol (real `tool_use` / `tool_result` content blocks and top-level tool definitions) instead of text-based simulation. Covers: mapping response `tool_use` blocks to `ToolCall`s and back, replaying multi-turn tool-use round-trips with correct role mapping and `tool_use_id` linkage, and passing the system prompt via the top-level `system` parameter on every turn.
