# langcat-provider Specification

## Purpose

LangCat 供应商（`langcat`）的接入——一个与现有 Claude / OpenAI / Ollama 并列的可选 AI 服务供应商，经 OpenAI 兼容 Chat Completions 协议访问 LongCat 大模型。覆盖供应商注册与配置读取、OpenAI 兼容请求构建（含 `thinking` 思考模式注入）、流式与非流式调用、响应解析与 reasoning 内容展示管线接入，以及 GUI 模型选择器与默认模型集成。
