...
### AI Agent 框架 (`org.qainsights.jmeter.ai.agent`)
...
### 工具层 (`org.qainsights.jmeter.ai.agent.tools`)
...
### 链路追踪 (`org.qainsights.jmeter.ai.tracing`)
...
- **CodeCommandHandler** - 处理 @code 命令（已禁用）
...
### 代码检查 (`org.qainsights.jmeter.ai.lint`)
- **LintCommandHandler** - 处理 @lint 命令
- **ElementRenamer** - 重命名元素以改善组织

### 优化 (`org.qainsights.jmeter.ai.optimizer`)
- **OptimizeRequestHandler** - 处理 @optimize 命令

### 包装 (`org.qainsights.jmeter.ai.wrap`)
- **WrapCommandHandler** - 处理 @wrap 命令
- **WrapUndoRedoHandler** - 包装操作的撤销/重做

### 使用统计 (`org.qainsights.jmeter.ai.usage`)
- **UsageCommandHandler** - 处理 @usage 命令
...
- **ClaudeCodeMenuItem** - Claude Code 面板的菜单项
- **TestPlanSerializer** - 将 JMeter 测试计划序列化为 Claude Code 上下文
- **JMeterActionBridge** - 桥接 JMeter 操作
- **DarkTerminalSettingsProvider** - 深色主题终端设置

### 终端 (`org.qainsights.jmeter.ai.terminal`)
- **DisabledTtyConnector** - 禁用状态下的 TTY 连接器

### 工具类 (`org.qainsights.jmeter.ai.utils`)
- **AiConfig** - AI 配置工具类
- **Models** - 模型常量定义
- **SystemPrompt** - 系统提示模板
- **TextUtils** - 文本处理工具
...
- **JMeterElementRequestHandler** - 处理元素请求

### 组件参数校验系统 (`org.qainsights.jmeter.ai.agent.validation`)
...
## 测试结构 (`src/test/java/org/qainsights/jmeter/ai/`)
...
| jediterm-* | 3.53 / 2.69 | 终端模拟器 |
| pty4j | 0.13.12 | PTY 实现 |
...
- **ELEMENT_CLASS_MAP** 注册了 134 个 JMeter 组件（涵盖采样器、线程组、断言、定时器、前置/后置处理器、配置元件、监听器、控制器等）
- 66 个组件拥有完整的 schema 定义和参考文档
...
**文件：** `src/main/java/org/qainsights/jmeter/ai/utils/JMeterElementManager.java`
