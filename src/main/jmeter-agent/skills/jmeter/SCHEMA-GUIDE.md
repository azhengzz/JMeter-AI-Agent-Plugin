...
| 场景 | YAML 改动 | Java 改动 |
|------|----------|----------|
| 为**已注册**的 JMeter 组件补/改 schema | ✅ 写 `.schema.yaml` 即可 | ❌ 不需要 |
| 新增一个 JMeter 里**完全不存在**的组件类型 | ✅ 写 `.schema.yaml` | ✅ 还要在 [`JMeterElementManager`](../../../src/main/java/org/gitee/jmeter/ai/utils/JMeterElementManager.java) 注册类映射 |

判断方法:打开 [`JMeterElementManager`](../../../src/main/java/org/gitee/jmeter/ai/utils/JMeterElementManager.java),在 `ELEMENT_CLASS_MAP` 里搜你的 elementType,找到 → 场景 1;找不到 → 场景 2。详见 [§13 Checklist](#13-新增组件-checklist)。
...
```yaml
component:
  type: debugsampler          # 小写 elementType,与 JMeterElementManager 注册 key 一致
  name: Debug Sampler         # 显示名
...
5. 全新组件(不在 `ELEMENT_CLASS_MAP` 里)还要改 Java,见 [§13 Checklist](#13-新增组件-checklist)

最简情况下只用到 `String` / `Integer` / `Boolean` 三种 type,不需要 `mountMode` / `class` / `itemProperties` 等高级字段。
...
JMeter 里几乎所有可序列化、可保存到 `.jmx` 的组件都实现 `TestElement` 接口 —— Sampler、Controller、Assertion、PreProcessor、ConfigElement、Listener 等都是。Schema 里 `class` / `itemClass` 字段填的就是某个 TestElement 子类的全限定名(如 `org.apache.jmeter.config.Arguments`)。
...
schema 里:
- 容器类填 `class`(如 `org.apache.jmeter.config.Arguments`)
...
```yaml
component:
  type: httpsampler            # 组件 elementType(小写)
  name: HTTP Request           # 显示名
  description: Sends HTTP...   # 用途说明
  aliases: [httprequest]       # 可选:别名 elementType 列表
  class: org.apache.jmeter...  # 可选:model 类全限定名(仅文档用)
...
- `component` 段:组件元信息,必填 `type`/`name`/`description`
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `type` | String | ✅ | 组件 elementType,小写唯一标识(如 `httpsampler`、`threadgroup`、`headermanager`)。被 `JMeterElementManager.ELEMENT_CLASS_MAP` 注册的 key |
| `name` | String | ✅ | 显示名(如 `HTTP Request`) |
| `description` | String | ✅ | 组件用途,供 AI 理解 |
| `aliases` | List&lt;String&gt; | ❌ | elementType 别名,允许通过多个名字加载同一 schema |
| `class` | String | ❌ | model 类全限定名,仅文档参考用(代码不读) |
...
| 字段 | 类型 | 用于模板 | 说明 |
|------|------|---------|------|
| `class` | String | container-items | 容器类全限定名(如 `org.apache.jmeter.config.Arguments`)。**Array 声明此字段即触发 container-items 模板**。与 [§6.3](#63-object-类型专用字段) Object 的 `class` 复用同一字段,含义由 `type` 决定(Array=容器类 / Object=嵌套对象类) |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `class` | String | 嵌套对象类全限定名(如 `org.apache.jmeter.control.LoopController`、`org.apache.jmeter.samplers.SampleSaveConfiguration`) |
...
│
├── ARRAY
│   │   读取 propDef.class / itemClass / itemProperties
│   ├─→ class != null            ──→  handleContainerItemsProperty(container-items 模板)
...
| 字符串列表(没有字段名) | string-collection | `type: Array`,不填 `class`/`itemClass` |
...
特征:**无** `class`,但有 `itemClass` + `itemProperties`,item 用无参构造 + setter,挂载为 `CollectionProperty`。
...
特征:`Array` 类型但**无** `class` / `itemClass`,直接存为字符串 `CollectionProperty`。
...
打开 [`JMeterElementManager`](../../../src/main/java/org/gitee/jmeter/ai/utils/JMeterElementManager.java),在 `ELEMENT_CLASS_MAP` 里搜你的组件 elementType:

- **能找到** → 这是"为已有组件补 schema",**跳过第 1 步**,直接做第 2 步
- **找不到** → 这是"全新组件",先做第 1 步注册

### 第 1 步:注册新组件(仅全新组件需要)

在 [`JMeterElementManager`](../../../src/main/java/org/gitee/jmeter/ai/utils/JMeterElementManager.java) 改两处:

1. **`ELEMENT_CLASS_MAP`** —— 加 elementType → (modelClass, guiClass) 映射
   ```java
   map.put("mynewsampler",
       new String[]{"com.example.MyNewSampler", "com.example.gui.MyNewSamplerGui"});
   ```
2. **`getDefaultNameForElement` switch** —— 加 case 返回默认显示名
   ```java
   case "mynewsampler": return "My New Sampler";
   ```
...
| 现象 | 可能原因 | 排查方向 |
|------|---------|---------|
| Agent 报"找不到组件类型" | `ELEMENT_CLASS_MAP` 没注册 / elementType 拼写不一致 | 第 1 步注册,且 schema `type` 与 map key 一致 |
...
- 组件注册:[JMeterElementManager.java](../../../src/main/java/org/gitee/jmeter/ai/utils/JMeterElementManager.java)
